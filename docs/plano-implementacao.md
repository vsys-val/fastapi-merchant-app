...
Planejamento consolidado. Entregas 1–5 implementadas e publicadas na branch `feat/estrutura-inicial`: fábrica FastAPI, configuração protegida, SQLAlchemy/Alembic, normalizadores, schemas, envelope de erros, autenticação e mutações de produtos. A criação detecta GTIN/identidade duplicados, a edição aplica autoria e bloqueio por avaliação de terceiros, e a exclusão lógica permite reativação controlada. O Supabase de desenvolvimento está em `0004_login_rate_limits`; as seis tabelas do schema público têm RLS. A suíte atual possui 87 testes aprovados e o fluxo de persistência de produtos foi validado no PostgreSQL com rollback. Antes de implantação pública, a aplicação ainda deverá receber um papel PostgreSQL próprio com privilégios mínimos. Entregas 6–8 ainda não foram implementadas.
