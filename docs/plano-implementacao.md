...
Planejamento consolidado. Entrega 1 em andamento na branch `feat/estrutura-inicial`: código de estrutura/configuração preparado, com sintaxe conferida; 7 testes locais passaram. O arquivo de testes ainda não foi publicado por bloqueio do limite automático de uso da conexão GitHub. Entrega 2 concluída localmente e publicada nesta branch: modelos SQLAlchemy, engine/sessão sob demanda e migração `0001_initial`. A configuração aceita URLs Supabase separadas para execução e migração, exige o uso documentado de SSL e ativa RLS. As migrações `0001_initial` e `0002_product_owner_index` foram aplicadas ao projeto Supabase de desenvolvimento. O teste transacional na nuvem validou relações, unicidade e CHECKs e terminou com rollback, mantendo as tabelas vazias. O advisor de segurança ainda aponta `public.alembic_version` sem RLS; a correção aguarda decisão explícita porque pode afetar o acesso da ferramenta de migração. Entregas 3–8 ainda não implementadas. Banco aprovado: PostgreSQL hospedado inicialmente no Supabase. Não há necessidade de continuar escolhendo isoladamente cada detalhe técnico para iniciar o desenvolvimento.
