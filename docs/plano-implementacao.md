...
Planejamento consolidado. Entregas 1–4 implementadas e publicadas na branch `feat/estrutura-inicial`: fábrica FastAPI, configuração protegida, SQLAlchemy/Alembic, normalizadores, schemas, envelope de erros, cadastro, login, JWT e `/users/me`. A limitação de login usa contadores atômicos no PostgreSQL e funciona entre múltiplos workers. O Supabase de desenvolvimento está em `0004_login_rate_limits`; as seis tabelas do schema público têm RLS, e `anon`/`authenticated` não possuem privilégios sobre `alembic_version` nem `limites_login`. A suíte atual possui 68 testes aprovados e o contador foi validado no banco com rollback. Antes de implantação pública, a aplicação ainda deverá receber um papel PostgreSQL próprio com privilégios mínimos. Entregas 5–8 ainda não foram implementadas.
